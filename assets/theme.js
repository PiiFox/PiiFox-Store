/*
@license
  Dawn by Shopify Themes (https://support.shopify.com)
...
if (console && console.log) {
  ...
  // 1. 定义 ASCII 字符画（可用 figlet 生成，这里预先准备）
  const asciiArt = `
         ███████╗  ██╗ ██╗ ██████╗╔███████╗  ██╗   ██ ╗
         ██╔══██╗  ██║ ██║ ██╔═══╝║██╔═  ██║  ╚██╗██╔╝
         ███████║  ██║ ██║ █████╗  ║██║    ██║    ╚███╔╝ 
...
(function () {
  ...
  theme.Drawers = (function () {
    ...
    Drawers.prototype = Object.assign({}, Drawers.prototype, {
      init: function () {
        ...
        const closeBtn = this.drawer.querySelector(this.config.close);
    ...
    parallax.prototype = Object.assign({}, parallax.prototype, {
      init: function (desktopOnly) {
        ...
        window.on(
          ...
          theme.utils.debounce(100, this.setSizes.bind(this)),
        );

        document.addEventListener(
          "shopify:section:reorder",
          theme.utils.debounce(100, this.onReorder.bind(this)),
        );
      },
  ...
  theme.AjaxProduct = (function () {
    ...
    ProductForm.prototype = Object.assign({}, ProductForm.prototype, {
      ...
      productAddToCartRest: function (addToCartEl, products) {
        return new Promise((resolve, reject) => {
  ...
  theme.headerNav = (function () {
    ...
    function closeSearchDrawer(evt) {
      ...
      document
        .querySelector("main.main-content")
        .removeEventListener("click", closeSearchDrawer);
    }
    ...
    Photoswipe.prototype = Object.assign({}, Photoswipe.prototype, {
      ...
      initGallery: function (items, index) {
        ...
        var options = {
          captionEl: false,
          counterEl: false,
          // 当前项目缩放时允许滑动导航到下一个/上一个项目
          allowPanToNext: false,
          // 在页面滚动时关闭图库
          closeOnScroll: false,
          // 如果设置为false禁用历史模块（后退按钮关闭图库，每张幻灯片的唯一 URL）
          history: false,
          // 启动幻灯片索引
          index: index - 1,
          // 捏合以关闭图库手势
          ...
          // 函数应返回一个对象，该对象具有初始放大动画开始（或缩小动画结束）的坐标
  ...
  theme.Collection = (function () {
    ...
    Collection.prototype = Object.assign({}, Collection.prototype, {
      ...
      initFilters: function () {
        ...
        document.querySelectorAll(selectors.clearTag).forEach((tag) => {
          tag.addEventListener("click", this.clearTagClick.bind(this));
        });
      ...
      clearTagClick: function (evt) {
        ...
        const formEls = document.querySelectorAll(selectors.tagsForm);
        formEls.forEach((form) => {
          const formData = new FormData(form);
          [...formData.keys()].forEach((key) => {
            formData.set(key, ""); // 把每个字段都设为空字符串
          });
          this.updateScroll(true);
          this.startLoading();
        ...
        // queryParams.set('sort_by', this.getSortValue()); // 保留排序

        // this.updateScroll(true);
        // this.startLoading();
        // this.renderCollectionPage(queryParams);
      },
      ...
      renderClearTag: function () {
        // If active tags List is empty, hide clear filters button
        const activeTagsEl = document.querySelectorAll(selectors.activeTags);
        if (activeTagsEl.length === 0) {
          document.querySelectorAll(selectors.clearTag).forEach((tag) => {
            tag.classList.add(classes.hide);
          });
        } else {
          document.querySelectorAll(selectors.clearTag).forEach((tag) => {
            tag.classList.remove(classes.hide);
          });
        }
      },

      renderPriceActiveTag: function (parent, el) {
        const textEl = parent.querySelector(".price-range__display-wrapper");
        ...
        if (parent.classList.contains(classes.activeTag)) {
          ...
          // If adding a tag, show new tag right away.
          // Otherwise, remove it before ajax finishes
          let priceRangeLi = el.closest("li");
          if (priceRangeLi.classList.contains(classes.removeTagParent)) {
            $(priceRangeLi).querySelector("a.btn--small").innerText =
              textEl.innerText;
          } else {
            // Append new tag in both drawer and sidebar
            document
  ...
  $(window).scroll(function () {
    var headt = $(window).scrollTop();
    if (headt >= 50) {
      $("#SiteHeader").addClass("light_bg");
    } else {
      $("#SiteHeader").removeClass("light_bg");
    }
  });
})();

// MixItUp plugin
$(".transition-body").mixitup({
  showOnLoad: "Features",
...
var swiper2 = new Swiper(".iconListSwiper", {
  slidesPerView: 3,
  spaceBetween: 20,
...
var swiper = new Swiper(".featureListSwiper", {
  slidesPerView: 3,
  spaceBetween: 20,
  navigation: {
    nextEl: ".fns",
...
var swiper = new Swiper(".featureListsSwiper", {
  slidesPerView: 2,
  spaceBetween: 10,
  navigation: {
    nextEl: ".ans",
...
var swiper = new Swiper(".featuSwiper", {
  slidesPerView: 4,
  spaceBetween: 30,
  breakpoints: {
    320: {
...
$(".tabs-nav__item-list .tabs-nav__item").click(function () {
  $(this).addClass("active").siblings().removeClass("active");
  var index = $(this).index();
  $(this)
    .parents(".page-width--flush-small")
    .find($(".featured-collections > div"))
    .eq(index)
    .show()
    .siblings()
    .hide();
...
$(document).ready(function () {
  $(".first_item li.active").parents(".first_item").addClass("active");
  $(".tab_nav ul li:first-child").addClass("active");
...
$(".row_one").click(function () {
  $(this)
    .parents(".collection-grid__wrapper")
    .find(".grid-product")
    .removeClass("small--one-halfs");
});
$(".row_two").click(function () {
  $(this)
    .parents(".collection-grid__wrapper")
    .find(".grid-product")
    .addClass("small--one-halfs");
});
$(".return-top, .m_product_addToCart, .pc_product_addToCart").click(
  function () {
...
$(window).scroll(function (e) {
  if ($(window).scrollTop() > 1300) $(".return-top").fadeIn(1000);
  else $(".return-top").fadeOut(1000);
});
(function () {
  const $window = $(window);
  const $filterSection = $(".filters-section");

  if (!$filterSection.length) return;

  let navfil = $filterSection.offset().top;

  const $mobileInfo = $(".mobile_bottominfo");
  const $filterSec = $(".filter-sec");
  const $productForm = $(".product-single__meta .product-single__form");

  function updateOffset() {
    navfil = $filterSection.offset().top;
  }

  function handleScroll() {
    const scroH = $window.scrollTop();
    const condition = scroH >= navfil;

    $mobileInfo[condition ? "show" : "hide"]();
    $filterSec.toggleClass("stickys", condition);
    $productForm.toggleClass("stickybtn", condition);
  }
  ...
  $window.on("resize load", updateOffset);
})();
$(".collapsible-t").click(function () {
  if ($(this).parents(".faq_items").hasClass("active")) {
    $(this).parents(".faq_items").removeClass("active");
  } else {
    $(this).parents(".faq_items").addClass("active");
  }
});
// 获取元素可见高度
function getVisibleHeight(el) {
  ...
  const elBottom = elOffsetTop + elHeight;
  const visibleTop = Math.max(0, elOffsetTop - scrollTop);
  const visibleBottom = Math.min(winHeight, elBottom - scrollTop);
